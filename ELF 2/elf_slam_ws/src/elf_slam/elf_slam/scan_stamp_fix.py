"""
LaserScan 时间戳和点数修正节点。

部分雷达驱动发布的 /scan 可能存在时间戳过旧、未来时间戳或 beams 数量不稳定的问题。
slam_toolbox/Nav2 对 TF 时间同步比较敏感，因此本节点把原始 scan 修正后发布为 /scan_fixed。
"""
...
class ScanStampFix(Node):
    """修正 LaserScan 的时间戳、角度一致性和可选点数重采样。"""
    ...
    def __init__(self):
        ...
        # 雷达数据属于高频传感器消息，使用 sensor_data QoS 可减少延迟。
    ...
    def _resample(self, src: LaserScan, target_beams: int) -> LaserScan:
        """
        将 LaserScan 重采样到固定点数。

        slam_toolbox 对 angle_min/angle_max/angle_increment 与 ranges 长度的一致性较敏感，
        固定点数可以避免雷达驱动偶发少点/多点造成建图抖动。
        """
        if target_beams <= 0:
            return src
        ...
        for i in range(target_beams):
            ...
            # 找到目标角度在原始 ranges 中的左右邻点，用线性插值生成新距离值。
            if src_f <= 0.0:
                idx0 = 0
                idx1 = 0
                t = 0.0
            ...
            r0 = src_ranges[idx0]
            r1 = src_ranges[idx1]
            # NaN/Inf 不能直接普通插值，否则会污染有效点；这里保留传感器语义。
            if r0 != r0 and r1 != r1:
                r = float('nan')
            elif r0 != r0:
                r = r1
    ...
    def _fix_angle_consistency(self, msg: LaserScan) -> LaserScan:
        """根据 ranges 长度重新计算 angle_increment 和 angle_max，保证角度字段自洽。"""
    ...
    def on_scan(self, msg: LaserScan):
        """接收原始 scan，修正时间和点数后发布。"""
        now = self.get_clock().now()
        stamp = rclpy.time.Time.from_msg(msg.header.stamp)
        age = (now - stamp).nanoseconds / 1e9

        # 时间戳为 0、过旧或明显来自未来时，直接改成当前 ROS 时间，避免 TF extrapolation 错误。
        ...
        # 允许给输出时间加一个负偏移，用于某些雷达驱动时间略超前的场景。
