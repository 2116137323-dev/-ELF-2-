...
import queue  # 导入 queue 模块，用于线程安全的事件队列
...
ROS2_AVAILABLE = False  # 标记 ROS2 依赖是否可用，默认不可用
ROS2_IMPORT_ERROR = None  # 保存 ROS2 导入失败时的异常，便于诊断
try:  # 尝试导入 ROS2 相关依赖
    import rclpy  # ROS2 Python 客户端库
    from rclpy.executors import SingleThreadedExecutor  # 单线程执行器，用于 spin 节点
    from rclpy.node import Node  # ROS2 节点基类
    from rclpy.qos import DurabilityPolicy, HistoryPolicy, QoSProfile, ReliabilityPolicy  # QoS 策略枚举
    from rclpy.time import Time as RosTime  # ROS 时间类型，别名 RosTime 避免冲突
    from geometry_msgs.msg import PoseStamped, Twist  # 位姿戳与速度消息类型
    from std_msgs.msg import Bool  # 布尔消息，用于急停信号
    from visualization_msgs.msg import Marker, MarkerArray  # RViz 可视化标记消息
    from tf2_ros import Buffer, TransformListener  # TF2 变换缓冲与监听器
    from action_msgs.srv import CancelGoal  # Nav2 取消导航目标的服务类型
    ROS2_AVAILABLE = True  # 全部导入成功，启用 ROS2 功能
except Exception as ros2_import_exc:  # 捕获任意导入异常
    ROS2_IMPORT_ERROR = ros2_import_exc  # 记录异常对象供后续日志输出
...
RTSP_URL = "rtsp://admin:123456@192.168.72.164:554/ch01.264"  # 可见光 RTSP 视频流地址
CAMERA_HTTP_HOST = "192.168.72.164:80"  # 摄像头 HTTP 控制主机地址
...
# 推理/后处理参数
OBJ_THRESH = 0.65  # 默认目标检测置信度阈值
NMS_THRESH = 0.45  # 非极大值抑制 IoU 阈值
BOX_HOLD_SEC = 0.6  # 检测框在画面上保留显示的时长(秒)
IMG_SIZE = (640, 640)  # 模型推理与 Web 推流的统一输入尺寸

# 类别映射与独立阈值
THERMAL_CLASS_THRESHOLDS = {0: 0.14}  # 热成像各类别对应的置信度阈值
CAMERA_CLASS_LABELS = {0: "PERSON", 1: "AMPUTATED_LIMB"}  # 可见光类别 ID 到名称的映射
CAMERA_CLASS_THRESHOLDS = {0: OBJ_THRESH, 1: 0.16}  # 可见光各类别独立置信度阈值
...
THERMAL_MIN_SPAN_C = 6.0  # 热成像渲染所需的最小温度跨度
THERMAL_CLAHE_CLIP = 2.5  # CLAHE 对比度限制的 clip 值
THERMAL_CLAHE_GRID = (8, 8)  # CLAHE 分块网格尺寸(列, 行)
ROS2_MAP_FRAME = "map"  # ROS 全局地图坐标系 frame_id
ROS2_BASE_FRAME = "base_footprint"  # 机器人基座坐标系 frame_id
ROS2_DETECTION_MARKER_TOPIC = "/dual_detect_markers"  # 检测定位点 MarkerArray 发布话题
ROS2_DETECTION_POSE_TOPIC = "/dual_detect_pose"  # 检测定位点位姿 PoseStamped 发布话题
ROS2_RVIZ_MARKER_TOPIC = "/visualization_marker"  # RViz 单 Marker 可视化话题
ROS2_RVIZ_MARKER_ARRAY_TOPIC = "/visualization_marker_array"  # RViz MarkerArray 可视化话题
ROS2_MAX_MARKERS = 100  # 内存中保留的最大历史检测定位点数量

# 报警后停止导航/停车相关配置
ROS2_CMD_VEL_TOPIC = "/cmd_vel"  # 机器人速度指令发布话题
ROS2_ESTOP_TOPIC = "/emergency_stop"  # 急停信号发布话题
ROS2_NAV_CANCEL_SERVICE = "/navigate_to_pose/_action/cancel_goal"  # Nav2 取消导航目标的服务名
STOP_NAV_ON_ALARM = True          # 报警时是否取消导航并停车
STOP_CMD_VEL_REPEAT = 10          # 报警后持续下发零速的次数(10Hz≈1秒)
...
ros_bridge_lock = threading.Lock()  # ROS 桥接对象的互斥锁
ros_bridge = {  # ROS2 桥接运行时状态
    'node': None,  # DetectionMarkerBridge 节点实例
    'executor': None,  # ROS2 单线程执行器实例
    'thread': None,  # 执行器 spin 所在的后台线程
}  # 结束 ros_bridge 字典


def detection_status_to_label(status):  # 将内部报警状态码转换为中文显示标签
    """将报警状态转换为 RViz 中更易读的简短标注。"""
    labels = {  # 状态码到中文标签的映射表
        "dual_detected": "被困人员",  # 双路同时检出
        "camera_detected": "死亡人员",  # 可见光检出人员
        "suspect_no_vital": "死亡人员",  # 嫌疑无生命体征
        "amputated_limb_with_temperature": "被埋人员",  # 残肢伴随温度
        "amputated_limb_no_temperature": "残肢",  # 残肢无温度
    }  # 结束中文标签映射表
    return labels.get(status, status.upper())  # 未知状态则返回大写原字符串


def detection_status_to_rviz_text(status):  # 将报警状态转换为 RViz ASCII 文本
    """RViz 文本标记优先使用 ASCII，避免中文在部分环境下不显示。"""
    labels = {  # 状态码到 RViz ASCII 标签的映射表
        "dual_detected": "SURVIVOR",  # 被困幸存者
        "camera_detected": "DECEASED",  # 死亡人员
        "suspect_no_vital": "DECEASED",  # 无生命体征嫌疑
        "amputated_limb_with_temperature": "BURIED",  # 被埋人员
        "amputated_limb_no_temperature": "LIMB",  # 残肢
    }  # 结束 RViz ASCII 标签映射表
    return labels.get(status, status.upper())  # 未知状态则返回大写原字符串


if ROS2_AVAILABLE:  # 仅在 ROS2 依赖可用时定义桥接节点类
    class DetectionMarkerBridge(Node):  # ROS2 节点：发布检测定位点到 RViz
        """在识别触发时，把车辆当前 map 坐标发布为 RViz 可见的定位点。"""

        def __init__(self):  # 初始化桥接节点、发布者与定时器
            super().__init__('dual_detect_marker_bridge')  # 调用父类构造，注册节点名
            marker_qos = QoSProfile(  # Marker 话题的 QoS 配置
                history=HistoryPolicy.KEEP_LAST,  # 仅保留最新消息
                depth=1,  # 队列深度为 1
                reliability=ReliabilityPolicy.RELIABLE,  # 可靠传输
                durability=DurabilityPolicy.TRANSIENT_LOCAL,  # 晚加入订阅者可收到最近一条
            )  # 结束 marker_qos 配置
            pose_qos = QoSProfile(  # 位姿话题的 QoS 配置
                history=HistoryPolicy.KEEP_LAST,  # 仅保留最新消息
                depth=10,  # 队列深度为 10
                reliability=ReliabilityPolicy.RELIABLE,  # 可靠传输
                durability=DurabilityPolicy.VOLATILE,  # 不保留历史，仅实时订阅
            )  # 结束 pose_qos 配置
            self.marker_pub = self.create_publisher(  # 创建检测点 MarkerArray 发布者
                MarkerArray, ROS2_DETECTION_MARKER_TOPIC, marker_qos  # 话题名与 QoS
            )  # 结束 marker_pub 创建
            self.rviz_marker_pub = self.create_publisher(  # 创建 RViz 单 Marker 发布者
                Marker, ROS2_RVIZ_MARKER_TOPIC, marker_qos  # 话题名与 QoS
            )  # 结束 rviz_marker_pub 创建
            self.rviz_marker_array_pub = self.create_publisher(  # 创建 RViz MarkerArray 发布者
                MarkerArray, ROS2_RVIZ_MARKER_ARRAY_TOPIC, marker_qos  # 话题名与 QoS
            )  # 结束 rviz_marker_array_pub 创建
            self.pose_pub = self.create_publisher(  # 创建检测点位姿发布者
                PoseStamped, ROS2_DETECTION_POSE_TOPIC, pose_qos  # 话题名与 QoS
            )  # 结束 pose_pub 创建
            self.cmd_vel_pub = self.create_publisher(Twist, ROS2_CMD_VEL_TOPIC, 10)  # 速度指令发布者
            estop_qos = QoSProfile(  # 急停话题的 QoS 配置
                history=HistoryPolicy.KEEP_LAST,  # 仅保留最新消息
                depth=1,  # 队列深度为 1
                reliability=ReliabilityPolicy.RELIABLE,  # 可靠传输
                durability=DurabilityPolicy.TRANSIENT_LOCAL,  # 晚加入订阅者可收到最近一条
            )  # 结束 estop_qos 配置
            self.estop_pub = self.create_publisher(  # 创建急停信号发布者
                Bool, ROS2_ESTOP_TOPIC, estop_qos  # 话题名与 QoS
            )  # 结束 estop_pub 创建
            self.nav_cancel_client = self.create_client(  # 创建 Nav2 取消导航服务客户端
                CancelGoal, ROS2_NAV_CANCEL_SERVICE  # 服务类型与名称
            )  # 结束 nav_cancel_client 创建
            self.tf_buffer = Buffer()  # 创建 TF2 变换查询缓冲
            self.tf_listener = TransformListener(self.tf_buffer, self, spin_thread=False)  # 监听 TF 变换
            self.event_queue = queue.Queue()  # 线程安全的检测事件队列
            self.marker_lock = threading.Lock()  # 保护 marker_points 列表的锁
            self.marker_points = []  # 已记录的历史检测定位点列表
            self.next_marker_id = 0  # 下一个 Marker 的唯一 ID
            self.last_tf_warn_time = 0.0  # 上次 TF 查询失败警告的时间戳
            self._stop_requested = False  # 是否已请求停车(由报警触发)
            self._stop_cmd_left = 0  # 剩余待下发的零速指令次数
            self.create_timer(0.2, self.process_detection_events)  # 每 0.2 秒处理检测事件队列
            self.create_timer(1.0, self.publish_marker_array)  # 每 1 秒发布完整 Marker 数组
            self.create_timer(0.1, self._stop_navigation_tick)  # 每 0.1 秒执行停车/取消导航逻辑
            self.get_logger().info(  # 记录节点启动日志
                f"识别定位点发布已启动: marker={ROS2_DETECTION_MARKER_TOPIC}, "  # Marker 话题名
                f"pose={ROS2_DETECTION_POSE_TOPIC}, rviz={ROS2_RVIZ_MARKER_TOPIC}"  # 位姿与 RViz 话题名
            )  # 结束 info 日志调用

        def enqueue_detection(self, status, timestamp):  # 将一次检测报警事件入队，供 ROS 线程处理
            # 报警即请求停止导航/停车，实际 ROS 调用在执行器线程内完成(保证线程安全)
            if STOP_NAV_ON_ALARM:  # 若配置为报警时停止导航
                # 急停信号立即发布(publish 线程安全)，底盘收到后直接写停止，
                # 绕过 cmd_vel/velocity_smoother 的减速斜坡，第一时间停车
                self.publish_estop(True)  # 立即发布急停 True，锁定停车
                self._stop_requested = True  # 标记需要在 tick 中取消导航并下发零速
            self.event_queue.put((status, timestamp))  # 将状态与时间戳放入事件队列

        def publish_estop(self, active):  # 发布或解除急停信号
            """发布/解除急停信号(True=急停锁定停车, False=解除恢复)。"""
            try:  # 尝试发布急停消息
                self.estop_pub.publish(Bool(data=bool(active)))  # 发布 Bool 急停状态
            except Exception as exc:  # 捕获发布异常
                self.get_logger().warning(f"发布急停信号失败: {exc}")  # 记录警告日志

        def _stop_navigation_tick(self):  # 定时回调：处理报警后的停车与导航取消
            """在执行器线程内处理报警停止：取消导航目标 + 持续下发零速停车。"""
            if self._stop_requested:  # 若有新的停车请求
                self._stop_requested = False  # 清除请求标志，避免重复触发
                self._stop_cmd_left = STOP_CMD_VEL_REPEAT  # 重置零速下发计数
                self._cancel_navigation()  # 异步取消 Nav2 导航目标
            if self._stop_cmd_left > 0:  # 若仍有待下发的零速次数
                self.cmd_vel_pub.publish(Twist())  # 全零速度，立即停车
                self._stop_cmd_left -= 1  # 递减剩余零速下发次数

        def _cancel_navigation(self):  # 调用 Nav2 服务取消当前所有导航目标
            """取消 Nav2 当前所有导航目标(空 goal_info 表示取消全部)。"""
            try:  # 尝试调用取消服务
                if self.nav_cancel_client.service_is_ready():  # 服务已就绪
                    self.nav_cancel_client.call_async(CancelGoal.Request())  # 异步发送空取消请求
                    self.get_logger().info("报警触发：已请求取消 Nav2 导航目标并停车")  # 记录成功日志
                else:  # 服务尚未就绪
                    self.get_logger().warning(  # 记录警告
                        "报警触发：Nav2 取消服务未就绪，已下发零速停车"  # 提示仅依赖零速停车
                    )  # 结束 warning 调用
            except Exception as exc:  # 捕获调用异常
                self.get_logger().warning(f"取消导航目标失败: {exc}")  # 记录失败原因

        def process_detection_events(self):  # 定时从队列取出检测事件并记录定位点
            updated = False  # 标记本轮是否有新事件被处理
            while True:  # 循环直到队列为空
                try:  # 尝试非阻塞取队
                    status, timestamp = self.event_queue.get_nowait()  # 取出状态与时间戳
                except queue.Empty:  # 队列已空
                    break  # 退出循环
                self._record_detection_point(status, timestamp)  # 查询 TF 并记录定位点
                updated = True  # 标记已有更新
            if updated:  # 若处理了新事件
                self.publish_marker_array()  # 立即发布完整 Marker 数组

        def _record_detection_point(self, status, timestamp):  # 查询当前位姿并保存检测定位点
            try:  # 尝试查询 map 到 base 的变换
                transform = self.tf_buffer.lookup_transform(  # 查找最新 TF 变换
                    ROS2_MAP_FRAME, ROS2_BASE_FRAME, RosTime()  # 源/目标坐标系与时间
                )  # 结束 lookup_transform 调用
            except Exception as exc:  # TF 查询失败
                now_sec = time.time()  # 当前 Unix 时间戳
                if now_sec - self.last_tf_warn_time >= 2.0:  # 距上次警告已超过 2 秒
                    self.get_logger().warning(  # 限频输出警告
                        f"无法获取 {ROS2_MAP_FRAME}->{ROS2_BASE_FRAME} 变换，未发布定位点: {exc}"  # 说明失败原因
                    )  # 结束 warning 调用
                    self.last_tf_warn_time = now_sec  # 更新上次警告时间
                return  # 无法定位则直接返回

            x = float(transform.transform.translation.x)  # 提取 map 坐标系 X
            y = float(transform.transform.translation.y)  # 提取 map 坐标系 Y
            z = float(transform.transform.translation.z)  # 提取 map 坐标系 Z
            label = detection_status_to_label(status)  # 转换为中文显示标签
            rviz_text = detection_status_to_rviz_text(status)  # 转换为 RViz ASCII 文本

            pose = PoseStamped()  # 构造位姿戳消息
            pose.header.frame_id = ROS2_MAP_FRAME  # 设置坐标系为 map
            pose.header.stamp = self.get_clock().now().to_msg()  # 设置当前 ROS 时间戳
            pose.pose.position.x = x  # 写入 X 位置
            pose.pose.position.y = y  # 写入 Y 位置
            pose.pose.position.z = z  # 写入 Z 位置
            pose.pose.orientation = transform.transform.rotation  # 复制机器人朝向
            self.pose_pub.publish(pose)  # 发布检测点位姿

            with self.marker_lock:  # 加锁保护 marker_points 列表
                self.marker_points.append({  # 追加新的历史定位点记录
                    'id': self.next_marker_id,  # 当前 Marker ID
                    'x': x,  # X 坐标
                    'y': y,  # Y 坐标
                    'z': z,  # Z 坐标
                    'label': label,  # 中文标签
                    'rviz_text': rviz_text,  # RViz 文本
                    'timestamp': timestamp,  # 报警时间戳
                })  # 结束定位点字典并 append
                self.next_marker_id += 1  # 递增下一个可用 ID
                if len(self.marker_points) > ROS2_MAX_MARKERS:  # 超出最大保留数量
                    self.marker_points = self.marker_points[-ROS2_MAX_MARKERS:]  # 截断保留最新 N 条

            self.get_logger().info(  # 记录定位点已保存
                f"已记录识别定位点: label={label}, x={x:.3f}, y={y:.3f}, ts={timestamp}"  # 输出标签与坐标
            )  # 结束 info 日志调用
            self.publish_latest_marker(  # 立即发布最新单点 Marker 到 RViz
                point_id=self.next_marker_id - 1, x=x, y=y, z=z, label=rviz_text  # 使用刚分配的 ID 与坐标
            )  # 结束 publish_latest_marker 调用

        def build_point_marker(self, point_id, x, y, z):  # 构建 RViz 球体定位点 Marker
            marker = Marker()  # 创建空 Marker 消息
            marker.header.frame_id = ROS2_MAP_FRAME  # 坐标系设为 map
            marker.header.stamp = self.get_clock().now().to_msg()  # 设置当前时间戳
            marker.ns = "dual_detect_points"  # Marker 命名空间
            marker.id = int(point_id)  # 唯一 Marker ID
            marker.type = Marker.SPHERE  # 类型为球体
            marker.action = Marker.ADD  # 操作为添加/更新
            marker.pose.position.x = x  # 球心 X 坐标
            marker.pose.position.y = y  # 球心 Y 坐标
            marker.pose.position.z = z + 0.18  # 球心 Z 坐标(略高于地面)
            marker.pose.orientation.w = 1.0  # 单位四元数，无旋转
            marker.scale.x = 0.18  # 球体 X 方向直径
            marker.scale.y = 0.18  # 球体 Y 方向直径
            marker.scale.z = 0.18  # 球体 Z 方向直径
            marker.color.a = 1.0  # 不透明度为 1(完全不透明)
            marker.color.r = 1.0  # 红色分量
            marker.color.g = 0.0  # 绿色分量
            marker.color.b = 0.0  # 蓝色分量(纯红球)
            return marker  # 返回构建好的球体 Marker

        def build_text_marker(self, point_id, x, y, z, label):  # 构建 RViz 文字标注 Marker
            text_marker = Marker()  # 创建空 Marker 消息
            text_marker.header.frame_id = ROS2_MAP_FRAME  # 坐标系设为 map
            text_marker.header.stamp = self.get_clock().now().to_msg()  # 设置当前时间戳
            text_marker.ns = "dual_detect_labels"  # 文字 Marker 命名空间
            text_marker.id = int(point_id) + 100000  # ID 偏移避免与球体 ID 冲突
            text_marker.type = Marker.TEXT_VIEW_FACING  # 类型为始终朝向相机的文字
            text_marker.action = Marker.ADD  # 操作为添加/更新
            text_marker.pose.position.x = x  # 文字锚点 X
            text_marker.pose.position.y = y  # 文字锚点 Y
            text_marker.pose.position.z = z + 0.55  # 文字锚点 Z(位于球体上方)
            text_marker.pose.orientation.w = 1.0  # 单位四元数
            text_marker.scale.z = 0.42  # 文字高度( TEXT_VIEW_FACING 用 scale.z)
            text_marker.color.a = 1.0  # 不透明度
            text_marker.color.r = 1.0  # 红色
            text_marker.color.g = 1.0  # 绿色
            text_marker.color.b = 0.0  # 蓝色(黄色文字)
            text_marker.text = label  # 显示的文字内容
            return text_marker  # 返回构建好的文字 Marker

        def publish_latest_marker(self, point_id, x, y, z, label):  # 即时发布最新单点与文字 Marker
            point_marker = self.build_point_marker(point_id, x, y, z)  # 构建球体 Marker
            text_marker = self.build_text_marker(point_id, x, y, z, label)  # 构建文字 Marker
            self.rviz_marker_pub.publish(point_marker)  # 发布球体到 RViz 单 Marker 话题
            self.rviz_marker_pub.publish(text_marker)  # 发布文字到 RViz 单 Marker 话题

        def publish_marker_array(self):  # 发布全部历史检测点的 MarkerArray
            with self.marker_lock:  # 加锁复制列表，避免长时间持锁
                marker_points = list(self.marker_points)  # 复制当前所有定位点
            if not marker_points:  # 若无历史点
                return  # 无需发布

            marker_array = MarkerArray()  # 创建空 MarkerArray
            stamp = self.get_clock().now().to_msg()  # 统一时间戳
            for point in marker_points:  # 遍历每个历史定位点
                marker = self.build_point_marker(  # 构建该点的球体 Marker
                    point_id=point['id'], x=point['x'], y=point['y'], z=point['z']  # 传入 ID 与坐标
                )  # 结束 build_point_marker 调用
                marker.header.stamp = stamp  # 覆盖为数组统一时间戳
                marker_array.markers.append(marker)  # 将球体 Marker 加入数组

                text_marker = self.build_text_marker(  # 构建该定位点的文字标注 Marker
                    point_id=point['id'],  # 传入定位点唯一 ID
                    x=point['x'],  # 传入定位点 X 坐标
                    y=point['y'],  # 传入定位点 Y 坐标
                    z=point['z'],  # 传入定位点 Z 坐标
                    label=point.get('rviz_text', point['label'])  # 优先使用 rviz 文本，否则用默认标签
                )  # build_text_marker 函数调用结束
                text_marker.header.stamp = stamp  # 设置文字 Marker 的时间戳
                marker_array.markers.append(text_marker)  # 将文字 Marker 加入数组

            self.marker_pub.publish(marker_array)  # 发布 Marker 数组到主话题
            self.rviz_marker_array_pub.publish(marker_array)  # 同步发布到 RViz 专用话题


def start_ros2_marker_bridge():  # 定义启动 ROS2 标注桥接后台线程的函数
    """启动 ROS2 后台线程，负责 TF 查询和定位点发布。"""  # 函数文档：启动 ROS2 线程处理 TF 与定位点
    if not ROS2_AVAILABLE:  # 若 ROS2 依赖不可用
        print(f"ROS2 标注功能不可用，跳过启动: {ROS2_IMPORT_ERROR}")  # 打印不可用原因并跳过
        return  # 直接返回，不启动桥接

    def _ros_spin():  # 定义 ROS2 事件循环的内部线程函数
        node = None  # 初始化节点引用为空
        executor = None  # 初始化执行器引用为空
        try:  # 尝试初始化并运行 ROS2
            rclpy.init(args=None)  # 初始化 rclpy，不传递命令行参数
            node = DetectionMarkerBridge()  # 创建检测定位点桥接节点
            executor = SingleThreadedExecutor()  # 创建单线程执行器
            executor.add_node(node)  # 将节点注册到执行器
            with ros_bridge_lock:  # 加锁更新全局桥接状态
                ros_bridge['node'] = node  # 保存节点引用供其他线程调用
                ros_bridge['executor'] = executor  # 保存执行器引用

            while state['running'] and rclpy.ok():  # 主循环：服务运行中且 ROS2 正常
                executor.spin_once(timeout_sec=0.2)  # 非阻塞处理一次回调，超时 0.2 秒
        except Exception as exc:  # 捕获 ROS2 线程中的任意异常
            print(f"ROS2 定位点线程异常: {exc}")  # 打印异常信息
        finally:  # 无论成功与否都执行清理
            with ros_bridge_lock:  # 加锁清空全局桥接引用
                ros_bridge['node'] = None  # 清除节点引用
                ros_bridge['executor'] = None  # 清除执行器引用
            if executor is not None and node is not None:  # 若执行器与节点均已创建
                try:  # 尝试从执行器移除节点
                    executor.remove_node(node)  # 注销节点，停止接收回调
                except Exception:  # 移除失败时忽略
                    pass  # 静默跳过
            if node is not None:  # 若节点仍存在
                try:  # 尝试销毁节点
                    node.destroy_node()  # 释放节点占用的 ROS2 资源
                except Exception:  # 销毁失败时忽略
                    pass  # 静默跳过
            try:  # 尝试关闭 rclpy
                if rclpy.ok():  # 若 rclpy 仍处于运行状态
                    rclpy.shutdown()  # 关闭 ROS2 上下文
            except Exception:  # 关闭失败时忽略
                pass  # 静默跳过

    bridge_thread = threading.Thread(target=_ros_spin, daemon=True)  # 创建守护线程运行 ROS2 循环
    bridge_thread.start()  # 启动桥接后台线程
    with ros_bridge_lock:  # 加锁记录线程句柄
        ros_bridge['thread'] = bridge_thread  # 保存线程引用供后续管理


def enqueue_detection_marker(status, timestamp):  # 定义向 ROS2 投递识别事件的函数
    """向 ROS2 后台线程投递一次识别事件，异步生成地图定位点。"""  # 函数文档：异步投递识别结果
    if not ROS2_AVAILABLE:  # 若 ROS2 不可用
        return  # 静默返回
    with ros_bridge_lock:  # 加锁读取桥接节点
        node = ros_bridge.get('node')  # 获取当前 ROS2 桥接节点
    if node is None:  # 若节点尚未就绪
        print("ROS2 定位点节点未就绪，本次识别未生成定位点")  # 提示节点未就绪
        return  # 放弃本次投递
    node.enqueue_detection(status, timestamp)  # 调用节点方法入队识别事件


def publish_emergency_stop(active):  # 定义发布/解除急停信号的函数
    """发布/解除底盘急停信号；ROS2 不可用时静默跳过。"""  # 函数文档：控制底盘急停
    if not ROS2_AVAILABLE:  # 若 ROS2 不可用
        return  # 静默跳过
    with ros_bridge_lock:  # 加锁读取桥接节点
        node = ros_bridge.get('node')  # 获取桥接节点引用
    if node is None:  # 若节点不存在
        return  # 无法发布，直接返回
    node.publish_estop(active)  # 向节点发送急停激活或解除指令
...
def build_thermal_web_infer_frame(img_therm, infer_size=IMG_SIZE):  # 定义构建 Web 推流一致推理帧的函数
    """生成与 Web 推流一致的热成像画面，再缩放到模型输入尺寸供 YOLO 识别。"""  # 函数文档：推流帧转推理输入
    if img_therm is None:  # 若无热图
        return np.zeros((infer_size[1], infer_size[0], 3), dtype=np.uint8)  # 返回黑色 BGR 占位图

    # 与 logic_and_render_thread / MJPEG 推流使用同一套显示渲染
    display_bgr = render_thermal_display_frame(img_therm, (640, 480))  # 按 Web 显示逻辑渲染 640x480
    if infer_size == (640, 480):  # 若推理尺寸已是显示尺寸
        return display_bgr  # 直接返回
    return cv2.resize(display_bgr, infer_size, interpolation=cv2.INTER_LINEAR)  # 双线性缩放到模型输入尺寸
...
if __name__ == '__main__':  # 脚本直接运行时进入主入口
    ...
    threading.Thread(target=logic_and_render_thread,  # 启动逻辑控制与渲染线程
                     daemon=True).start()  # 守护线程
    start_ros2_marker_bridge()  # 启动 ROS2 检测标记桥接
